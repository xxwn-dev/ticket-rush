...
import com.xxwn.ticket_rush.domain.booking.BookingController;
...
class BookingControllerSliceTest {
    ...
    void setUp() {
        ...
        when(redisTemplate.hasKey(contains("active_user:"))).thenReturn(true);
        // 기본: 예매 진행 중 없음 (중복 예매 체크 통과)
        when(redisTemplate.hasKey(contains("booking:result:"))).thenReturn(false);
    }

    // ── V2 예매 ──────────────────────────────────────────────────────────

    @Test
    @DisplayName("V2 예매: 좌석이 가용하면 200 OK + seatId 반환")
    void createBookingV2_success() throws Exception {
        // RANDOM_RESERVE_LUA는 String(seatId) 반환
    ...
    @Test
    @DisplayName("V2 예매: 매진이면 409 Conflict 반환")
    void createBookingV2_soldOut() throws Exception {
        // SRANDMEMBER가 nil 반환 → null
    ...
    @Test
    @DisplayName("V2 예매: 매진 시 409 Conflict (duplicateUserBooking 케이스)")
    void createBookingV2_duplicateUserBooking() throws Exception {
        // booking:result:{userId} 이미 존재 → Lua가 nil 반환 (SETEX 조건 실패)
        when(redisTemplate.execute(any(), anyList(), any(), any())).thenReturn(null);
    ...
    @Test
    @DisplayName("V1 예매: 이미 예매 진행 중인 유저가 요청하면 409 Conflict 반환")
    void createBookingV1_duplicateUserBooking() throws Exception {
        // booking:result:{userId} 가 이미 존재 → 중복 예매 거부
        when(redisTemplate.hasKey("booking:result:1")).thenReturn(true);

        mockMvc.perform(post("/api/v1/bookings")
                        .header("X-USER-ID", "1")
                        .contentType("application/json")
                        .content("{\"eventId\":1,\"seatId\":10}"))
                .andExpect(status().isConflict());
    }
    ...
    void getAvailableSeats_returnsList() throws Exception {
        when(setOps.members("event:1:available")).thenReturn(Set.of("10", "11", "12"));
