...
import com.xxwn.ticket_rush.domain.booking.*;
...
class BookingServiceTest {
    ...
    void createBooking_success() {
        when(seatRepository.findByIdAndEventId(1L, 1L)).thenReturn(Optional.of(seat));
        when(bookingRepository.save(any())).thenReturn(Booking.create(USER_ID, seat));

        assertThatCode(() -> bookingService.createBooking(USER_ID, 1L, 1L))
    ...
    void createBooking_seatNotFound() {
        when(seatRepository.findByIdAndEventId(99L, 1L)).thenReturn(Optional.empty());

        assertThatThrownBy(() -> bookingService.createBooking(USER_ID, 99L, 1L))
    ...
    void processV2Booking_success() {
        ...
        bookingService.processV2Booking(USER_ID, 1L, 1L);

        verify(valueOps).set(eq("booking:result:" + USER_ID), eq("PAYMENT_PENDING"), eq(600L), eq(TimeUnit.SECONDS));
    }
    ...
    void processV2Booking_seatNotFound() {
        ...
        assertThatThrownBy(() -> bookingService.processV2Booking(USER_ID, 99L, 1L))
                .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void processCancel_success() {
        when(redisTemplate.opsForValue()).thenReturn(valueOps);
        when(seatRepository.findByIdAndEventId(1L, 1L)).thenReturn(Optional.of(seat));
        when(bookingRepository.deleteByUserIdAndSeatId(USER_ID, 1L)).thenReturn(1L);

        bookingService.processCancel(USER_ID, 1L, 1L);

        verify(valueOps).set(eq("booking:result:" + USER_ID), eq("CANCELLED"), eq(600L), eq(TimeUnit.SECONDS));
    }
    ...
    void processCancel_alreadyCancelled() {
        when(seatRepository.findByIdAndEventId(1L, 1L)).thenReturn(Optional.of(seat));
        when(bookingRepository.deleteByUserIdAndSeatId(USER_ID, 1L)).thenReturn(0L);
