...
import com.xxwn.ticketing_app.domain.queue.WaitingQueueService;
...
class WaitingQueueServiceTest {
    ...
    void registerQueue_addsUserToZSet() {
        waitingQueueService.registerQueue(42L);

        verify(zSetOps).add(eq("concert:waiting_queue"), eq("42"), anyDouble());
    }
    ...
    void registerQueue_scoreIsPositive() {
        ...
        waitingQueueService.registerQueue(1L);

        verify(zSetOps).add(eq("concert:waiting_queue"), eq("1"), scoreCaptor.capture());
    ...
    void registerQueue_differentUsersHaveIncreasingScores() throws InterruptedException {
        ...
        waitingQueueService.registerQueue(1L);
        Thread.sleep(2);
        waitingQueueService.registerQueue(2L);

        verify(zSetOps, times(2)).add(eq("concert:waiting_queue"), anyString(), scoreCaptor.capture());
