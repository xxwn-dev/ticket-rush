...
public class BookingConsumer {
    ...
    public void receiveV1Message(BookingMessage message) {
        ...
        try {
            redisLockFacade.createBookingWithLock(message.userId(), message.seatId(), message.eventId());
    ...
    public void receiveV2Message(BookingMessage message) {
        ...
        String cacheKey = "event:" + message.eventId() + ":available";
        String resultKey = "booking:result:" + message.userId();
        try {
            bookingService.processV2Booking(message.userId(), message.seatId(), message.eventId());
        } catch (IllegalStateException e) {
    ...
    public void receiveHoldExpiredMessage(BookingMessage message) {
        ...
        try {
            bookingService.releaseExpiredHold(message.userId(), message.seatId(), message.eventId());
        } catch (Exception e) {
    ...
    public void receiveCancelMessage(BookingMessage message){
        ...
        try {
            bookingService.processCancel(message.userId(), message.seatId(), message.eventId());
        } catch (Exception e) {
