...
import com.xxwn.ticketing_app.config.RedisConfig;
...
public class QueueScheduler {
    ...
    private static final int ALLOW_COUNT = 2000;
        ...
        for (int i = 0; i < totalWait; i++) {
            ...
            Thread.startVirtualThread(() -> {
                try {
                    ...
                    if (sseService.exists(userId)) {
                        long rank = index + 1;
                        sseService.sendStatusUpdate(userId, new QueueResponse(rank, totalWait - rank, "WAITING"));
                    }
                } catch (Exception e) {
